...
def balanced(m):
    ...
    if is_planet(m):
        ...
        torque_left = length(left_arm) * total_weight(left_end)
        torque_right = length(right_arm) * total_weight(right_end)
...
def totals_tree(m):
    ...
    def solve(m):
        if is_planet(m):
            ...
            return tree(total_weight(m), [solve(left_end), solve(right_end)])
    return solve(m)
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    def solve(m):
        if is_leaf(m):
            return tree(lokis_replacement) if label(m) == 'loki' else m
        else:
            return tree(label(m), [solve(b) for b in branches(m)])
    return solve(t)
...
def has_path(t, word):
    ...
    def solve(t, word):
        ...
        if is_leaf(t):
            return label(t) == word
        for b in branches(t):
            if label(t) == word[0] and solve(b, word[1:]):
                return True
        return False
    return solve(t, word)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not (lower_bound(y) <= 0 <= upper_bound(y)), 'Diveid by zero'
