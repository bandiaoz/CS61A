...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            cnt += 1
    if max(len(typed_words), len(source_words)) == 0:
        return 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if word == typed_word:
            return word
        if dif > diff_function(typed_word, word, limit):
            dif = diff_function(typed_word, word, limit)
            res = word
    return res if dif <= limit else typed_word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0:  # Base cases should go here, you may add more base cases as needed.
        return 0
    if len(typed) == 0 or len(source) == 0:
        return min(limit + 1, len(typed) + len(source))
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    upload({'id': user_id, 'progress': cnt / len(prompt)})
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    all_times = [[] for _ in range(len(times_per_player))]
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_all_times(match)[j][i] < min_time:
                ...
                min_index = j
        ans[min_index].append(get_all_words(match)[i])
    return ans
    # END PROBLEM 10
