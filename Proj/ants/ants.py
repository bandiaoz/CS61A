...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        def check(place, distance):
            return place.bees != [] and not place.is_hive and self.lower_bound <= distance <= self.upper_bound
        
        distance = 0
        while place is not None and not check(place, distance):
            place = place.entrance
            distance += 1
        return random_bee(place.bees) if place is not None else None
        # END Problem 3 and 4
